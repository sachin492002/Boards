...
import {deleteBoard, deleteBookmark} from "../Store/boardReducer";

export default  function Board({board,type}){
    const [modal, setModal] = useState(false);
    const dispatch = useDispatch();
    const {boardModalOpen} = useSelector((state) => state.Boards);

    const handleDelte=(event)=>{
        event.preventDefault()
        if(type==='bookmark')
            dispatch(deleteBookmark(board.title))
        else
            dispatch(deleteBoard(board.title))
    }
    return(
        <div className={boardModalOpen?'':'relative'}>
            <Link to={`board/${board?.title}`} className='inline-flex w-full items-center justify-between border border-1 rounded-lg'>
                <div className='inline-flex items-center  gap-10'>
                    <div className='p-10 w-8 h-8 rounded-l-lg'
                         style={{backgroundColor: `var(${board.color})`}}
                    ></div>
                    <p className='line-clamp-1'>{board.title}</p>
                </div>
                <PiDotsThreeVertical className='text-[#222222] mr-4 cursor-pointer' onClick={(e)=>{e.preventDefault();setModal(!modal);}}/></Link>
            {modal &&
                <div className='absolute flex flex-col box-border border-1 p-4 rounded-lg gap-4 -right-20 -bottom-20 bg-white shadow-xl'>
